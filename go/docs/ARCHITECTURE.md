...
| `adapters/driven/llm/resilience` | `xai-circuit-breaker` | A `Provider` decorator wrapping any `ports.LLMProvider` in a circuit breaker backed by the official `github.com/sony/gobreaker/v2` (`TwoStepCircuitBreaker`, CLOSED/OPEN/HALF_OPEN, lazy recovery on read) — not hand-rolled. Observes the full event stream (not just the synchronous return) so a mid-stream `EventError` still counts as a failure. Always wraps the selected provider in `cmd/grok/main.go` (fixed threshold/reset timeout default — no per-provider config yet) |
